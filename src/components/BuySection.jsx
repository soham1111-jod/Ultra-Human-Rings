...
const BuySection = forwardRef((props, ref) => {
  const { selectedRing, updateSelection, handleCheckout } = useRing()
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-8">
              ...
              >
                {ringOptions.colors.map(color => (
                  <motion.button
                    ...
                    className={`relative p-4 rounded-xl border-2 transition-all ${selectedRing.color === color.id
                        ? 'border-blue-500'
                        : 'border-gray-200'
                      }`}
            ...
            {/* Add Size Selection */}
            <div role="region" aria-label="Ring size selection" className="mt-8">
              <h2 id="size-selection" className="text-title font-semibold tracking-tight">Size.</h2>
              <div
                role="radiogroup"
                aria-labelledby="size-selection"
                className="grid grid-cols-4 gap-3 mt-4"
              >
                {ringOptions.sizes.map(size => (
                  <motion.button
                    key={size}
                    onClick={() => updateSelection('size', size)}
                    className={`py-3 px-4 rounded-lg border transition-all ${
                      selectedRing.size === size
                    ...
                    whileTap={{ scale: 0.98 }}
                    role="radio"
                    aria-checked={selectedRing.size === size}
            ...
            <div className="mt-8">
              <h2 className="text-title font-semibold tracking-tight mb-4">Quantity</h2>
              <div className="flex items-center gap-4">
                <motion.button
                  onClick={() => updateSelection('quantity', Math.max(1, selectedRing.quantity - 1))}
                ...
                <span className="text-lg font-medium">{selectedRing.quantity}</span>
                <motion.button
                  onClick={() => updateSelection('quantity', selectedRing.quantity + 1)}
                  className="p-2 rounded-lg border border-gray-200"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
